...
function bus() {
	...
	$.get(burl, function(data) {
		$('#busAlert').fadeOut(1000);
		$(data).find('prd').each(function() {
			var time = $(this).find('prdtm').text();
			var train = new Date(parseInt(time.substr(0,4)), parseInt((time.substr(4,2) - 1)), parseInt(time.substr(6,2)), (parseInt((time.substr(9,2))) + window.timezoneOffset), parseInt(time.substr(12,2)));
			var now = new Date();
			var when = train - now;
		...
		$('#busList').fadeOut(1000);
		$('#busAlert').fadeIn(1000);
