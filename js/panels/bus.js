	...
	$.get(burl, function(data) {
		$(data).find('prd').each(function() {
			...
			if (!(parseInt($(this).find('rt').text()) == 18 && parseInt($(this).find('stpid').text()) == 1580) && when > 0) {
				buslist.append('<li>' + $(this).find('rt').text() + '<span class="dest">' + $(this).find('des').text().replace(/(\d)(th|rd)/,'$1<span class="th">$2</span>') + '</span> <span class="min">' + (when < 1 ? 'Due' : when) + '</span></li>');
			}
		});
